...
- Decision 2026-02-17 p12-yolo-mode: Approvals policy is a 5-mode system including `yolo` (supersedes f6a7b8c)
    Decision: `approvals.mode = "yolo"` auto-approves commands + MCP (like `all`) and sends full-auto flags to supporting clients: Claude `--dangerously-skip-permissions`, Codex `approval_policy=never` + `sandbox_mode=danger-full-access`, VS Code `chat.tools.global.autoApprove=true`, Copilot CLI `--yolo`. A one-line `[yolo]` stderr acknowledgement runs on every sync/launch.
    Reason: Sandboxed/ephemeral environments want to skip all prompts without per-client setup.
    Tradeoffs: YOLO bypasses all safety prompts only where clients expose the control; Antigravity gets managed permissions but no yolo launch flag. No `al doctor` warning — YOLO is a deliberate choice, not a health issue.
...
- Decision 2026-02-18 config-resilience: Strict runtime parse + lenient repair path (supersedes config-migrate, unknown-key-repairable)
    ...
    Tradeoffs: Users must run `al wizard` or `al upgrade` to fix broken configs instead of auto-repair.
...
- Decision 2026-05-07 reasoning-effort-capability-matrix: Reasoning effort is per-client and custom-tolerant where upstream cadence demands it
    Decision: Support Codex and Claude `reasoning_effort` as custom-tolerant fields; reject Copilot CLI effort until a verified control exists; for Claude, require Opus, pass all values via `--effort`, and write settings `effortLevel` only for non-`max` values.
    ...
    Tradeoffs: Typos surface as sync warnings instead of hard errors for Claude/Codex; Copilot configs still fail fast until support is intentionally added.

- Decision 2026-05-09 claude-agent-specific-deep-merge: Claude agent_specific deep-merge; install seed denies AskUserQuestion
    Decision: Claude `agent_specific` is deep-merged into `.claude/settings.json` for object values (arrays/scalars still replace). `permissions.deny` is additive and silent; `permissions.allow` and `effortLevel` still warn on override. Install seed ships `agent_specific.permissions.deny = ["AskUserQuestion"]` to disable Claude Code's clarification tool by default. Codex `agent_specific` remains shallow.
    ...
    Tradeoffs: Claude override-warning logic is bespoke (`claudeAgentSpecificOverrideWarning`). `cloneClaudeSettingValue` only deep-clones the projected types (`map[string]any`, `[]any`, `[]string`); new slice types need explicit support.
