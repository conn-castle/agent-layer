...
- Decision 2026-05-16 skill-architecture: Diff-scoped cleanup is two skills; reviewer rubrics live beside the owning skill
    Decision: (a) Two narrow leaf skills (`prune-new-tests` + `simplify-new-code`) sit between implementation and verification; the codebase-scoped sibling is `simplify-codebase`. (b) Re-grading workflows use reviewer rubrics that prevent the implementer's narrative from becoming the proof of completion; `verify-work` keeps a plan/request-anchored comparison rubric in sibling `contract-verification-rubric.md` while remaining a hermetic root skill that does not call another skill. (c) Verbatim reviewer prompts or rubrics live in sibling files; the owning `SKILL.md` reads and applies them as part of its own contract. (d) Bounded accepted-finding fixes are ordinary agent or workflow behavior, not a standalone root skill; `resolve-findings` remains an orchestration skill for review-report triage, accepted-fix grouping, and resolution ledgers.
    ...
    Tradeoffs: Larger skill catalog and more re-reads (wall-clock + tokens) for fresh-context invocations. Report-shaped workflows keep a separate resolver instead of duplicating finding verdict bookkeeping in each orchestrator, but direct fixes stay in normal agent behavior instead of adding a generic fixer skill. `audit-and-fix-uncommitted-changes` Round N+1 and `resolve-findings` Phase 5 Auditor are intentionally out of scope (already bounded by external artifacts).
