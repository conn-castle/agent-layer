...
- Decision 2026-07-13 dispatch-depth-three: Root-to-leaf dispatch with adaptive provider-native plan review
    Decision: Keep `dispatch.max_depth = 3` for intentional custom workflows and keep built-in workflows root-to-leaf in Agent Dispatch. Each external plan reviewer must independently choose 1–4 fresh built-in review subagents, report the count, rationale, and distinct full-plan framings, and include an architecture framing for consequential architecture changes; those subagents cannot call Agent Dispatch or another workflow.
    Reason: Nested dispatch relay agents caused polling, duplicated evidence, and ambiguous lifecycle ownership, while flattening plan review entirely removed useful adaptive perspectives for broad or risky plans.
    Tradeoffs: Plan review has variable provider-native cost and latency, bounded by four subagents per reviewer; explicit strategy evidence makes that cost auditable without restoring fixed three-by-three dispatch fanout.
...
- Decision 2026-07-14 global-notifications-chime: One filtered handler serves project-shareable provider hooks
    Decision: `notifications.chime` remains the single typed opt-in; sync projects `al hook chime <provider>` into ordinary Claude, Codex, and Agent Layer-owned Antigravity `Stop` hooks, where typed event filters accept only known top-level, non-background, normal completion states and every projected command fails open.
...
- Decision 2026-07-17 claude-lineage-derived-proof: Claude lineage capability is version-gated and summaries stay derived
    Decision: Keep Claude 2.1.207 as the tested dispatch baseline, gate `--forward-subagent-text` and normalized lineage capture separately at 2.1.211, store one append-only `provider.lineage` artifact, and derive Claude-only descendant summaries from it on inspection/history.
    ...
    Tradeoffs: Missing, malformed, conflicting, or discontinuous evidence remains explicitly unknown; old and unsupported-provider records require no migration but cannot gain proof retroactively.
