...
- Decision 2026-05-16 skill-architecture: Diff cleanup and accepted-finding fixes stay workflow-owned
    ...
    Reason: Tests are agent side-effects (delete by default) while added code was user-requested (preserve; remove only scope creep) — opposite dispositions degenerate to a justification framework if one rubric covers both. Self-grading inherits primacy/self-consistency biases; a rubric-only review is the cheapest fix. Review reports now keep all candidate findings visible by recommended verdict, while fix workflows verify and fix `Recommended Accept` findings directly.
