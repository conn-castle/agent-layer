...
- Decision 2026-05-16 skill-architecture: Diff cleanup lives inside clean-and-fix-code assets
    Decision: (a) Diff-scoped cleanup workflows (`prune-new-tests` and `simplify-new-code`) are assets under `clean-and-fix-code`, not standalone root skills; `simplify-codebase` remains the codebase-wide sibling. (b) Re-grading workflows use reviewer rubrics that prevent the implementer's narrative from becoming the proof of completion; `verify-work` keeps a plan/request-anchored comparison rubric in sibling `contract-verification-rubric.md` while remaining a hermetic root skill that does not call another skill. (c) Verbatim reviewer prompts or rubrics live as files owned by the workflow that applies them. (d) Bounded accepted-finding fixes are ordinary agent or workflow behavior, not a standalone root skill; `resolve-findings` remains an orchestration skill for review-report triage, accepted-fix grouping, and resolution ledgers.
    Reason: Tests are agent side-effects (delete by default) while added code was user-requested (preserve; remove only scope creep) — opposite dispositions degenerate to a justification framework if one rubric covers both. Self-grading inherits primacy/self-consistency biases; a rubric-only review is the cheapest fix. Keeping the diff cleanup workflows as assets preserves the fresh-context rubric behavior without expanding the root skill catalog.
    Tradeoffs: Smaller skill catalog and fewer root-skill re-reads, but direct invocation of those cleanup passes now goes through `clean-and-fix-code`. Report-shaped workflows keep a separate resolver instead of duplicating finding verdict bookkeeping in each orchestrator, but direct fixes stay in normal agent behavior instead of adding a generic fixer skill.
