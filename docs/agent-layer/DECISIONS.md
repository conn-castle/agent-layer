...
- Decision 2026-05-28 cli-skill-catalog: Bare init ships no catalog or workflow bundle; wizard installs optional workflow/catalog content
    Decision: The four CLI-wrapper skills (`tavily-web`, `playwright-cli`, `find-docs`, `agent-dispatch`) live in a wizard-only embedded catalog at `internal/templates/skills-catalog/` keyed by `internal/templates/cli-skills-catalog.toml`. Bare `al init` installs no catalog skills, workflow skills, instruction files, memory docs/templates, or marker file; it creates empty `.agent-layer/instructions/` and `.agent-layer/skills/`. Wizard Q1=yes installs or refreshes the workflow bundle by overwriting managed bundled instructions and workflow skill dirs while creating missing `04_conventions.md` and memory docs/templates. Wizard Q1=no is a no-op for workflow files. The separate catalog multiselect copies selected catalog skill dirs and removes deselected catalog ids.
    Reason: Direct precedent is `mcp-catalog-seed-split` (2026-05-07). CLI-wrapper skills require external binaries the user may not have; bundling them by default produced silent breakage. The older alternate-layout flag, marker file, and workflow deletion semantics created a second install mode and a surprising delete path, so bare init is now the universal scaffold and wizard opt-in is the only bundle install/refresh action.
    Tradeoffs: Existing repos keep their catalog skill directories on upgrade (the ownership classifier marks them `catalog_skills_v1`); deselecting a catalog id in the wizard removes that catalog directory after diff-preview confirmation. Workflow-bundle refresh can replace managed bundled instructions/skills, but it preserves user-owned `04_conventions.md`, existing memory files/templates, user-created skills, and all workflow files when Q1=no.
