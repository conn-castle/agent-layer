...
- Decision 2026-05-16 skill-architecture: Diff cleanup and accepted-finding fixes stay workflow-owned
    Decision: (a) Diff-scoped cleanup workflows (`prune-uncommitted-tests` and `simplify-uncommitted-code`) are assets under `clean-and-fix-code`, not standalone root skills; `simplify-codebase` remains the codebase-wide sibling. (b) Re-grading workflows use reviewer rubrics that prevent the implementer's narrative from becoming the proof of completion; `verify-work` keeps a plan/request-anchored comparison rubric in sibling `contract-verification-rubric.md` while remaining a hermetic root skill that does not call another skill. (c) Verbatim reviewer prompts or rubrics live as files owned by the workflow that applies them. (d) Review verdict classification lives in `review-uncommitted-code/assets/finding-verdict-classification.md`; bounded accepted-finding fixes are ordinary orchestrator/agent behavior in workflows that apply them, not a standalone root skill.
