...
- Decision 2026-06-02 provider-statusline-sources: Statusline toggles are per-agent booleans with provider-specific editable sources
    Decision: `agents.claude.statusline` and `agents.codex.statusline` are opt-out booleans (`nil`/`true` enable, explicit `false` disables). Claude uses `.agent-layer/claude-statusline.sh` projected to `.claude/claude-statusline.sh`; legacy `.agent-layer/statusline.sh` migrates write-if-missing. Codex uses `.agent-layer/codex-statusline.toml` as a statusline-only `[tui].status_line` fragment merged into `.codex/config.toml`, unless `agents.codex.agent_specific.tui.status_line` is explicit.
    Reason: A single unified statusline schema made end-user customization too hard and would cap Claude-specific script flexibility. Boolean toggles keep Agent Layer responsible for provider wiring while users edit each provider-native source directly.
    Tradeoffs: Shipped source improvements reach only new or missing source files. Codex cannot mirror Claude script-only fields such as ANSI formatting, cost, lines changed, worktree label, or PR review state. The `0.11.0` manifest still carries forward the 0.10.2 Gemini→Antigravity ops because unpinned upgrades load only the latest manifest — duplication tracked in ISSUES.md (`unpinned-upgrade-latest-manifest-only`).
