...
- Decision 2026-05-31 wizard-feature-disable-agent-specific: Per-feature disable toggles store direct `agent_specific` keys, not a translated central flag
    Decision: The wizard "disable" toggles (Codex browser/computer-use, Codex apps, Claude IDE open-file reading, Claude auto-memory, Claude connectors, Claude AskUserQuestion) write the client-native key straight into the agent's `agent_specific` passthrough — Codex `features.*`, Claude `env.*`/`autoMemoryEnabled`/`permissions.deny`/`hooks.PreToolUse`. No typed config field, no `fields.go` entry, no sync changes. A key is written only when the user opts in (disable intent); leaving a toggle off omits the key so the client keeps its own default (no hidden defaults). Patch writers `applyCodexBrowserUpdate` and `applyClaudeAgentSpecificUpdate` parallel `applyCodexAppsUpdate`.
    Reason: Generated client files already derive entirely from `agent_specific` (Claude deep-merge, Codex append), so a typed field would create a second canonical source and duplicate sync logic. Supersedes the BACKLOG `disable-client-memory-config` proposal of a single translated flag.
...
- Decision 2026-05-31 askuserquestion-deny-plus-hook-default-allowed: AskUserQuestion blocked via deny + PreToolUse hook; template no longer disables it by default
    Decision: The shipped template no longer seeds `permissions.deny = ["AskUserQuestion"]`, so fresh `al init` allows the tool (Claude Code's native default). The wizard's opt-in toggle, when on, writes BOTH `permissions.deny = ["AskUserQuestion"]` and a `PreToolUse` hook (`matcher = "AskUserQuestion"`, command exits 2).
    Reason: `permissions.deny` is skipped entirely under YOLO/`bypassPermissions` (Claude Code permission-modes docs), so the old template-only deny silently failed under YOLO. PreToolUse hooks fire in every mode, so the hook is the robust block; the deny is the native non-YOLO mechanism. Defaulting to allowed avoids shipping a hidden behavior.
    Tradeoffs: New installs lose the previous default-off behavior (CHANGELOG call-out); existing repos keep their deny (config.toml is never overwritten on upgrade) and migrate via `al wizard`. The toggle "owns" the deny and hook lines, so manually-merged extra entries there are overwritten when toggling.
