...
- The public method and website brand is **DeltaSelect**. Use `DeltaSelect` for the top-level page, navigation, metadata, and explanatory copy; DeepSWE is the first public evidence dataset and may remain in the established `/deepswe-planner` implementation route. DeltaSelect is planned for v0.18.0 alongside its companion arXiv paper; the paper is not public yet.
