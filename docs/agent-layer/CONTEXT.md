...
- The public method and website brand is **DeltaSelect**. Use `DeltaSelect` for the top-level page, navigation, metadata, and explanatory copy. The interactive page is `/deltaselect-tool`; DeepSWE is the first public evidence dataset, and its static implementation assets remain under `site/static/deepswe-planner/`.
