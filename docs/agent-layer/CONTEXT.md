...
- The website planner is the canonical task-selection surface. Its published calibration reference and contrast select responsive tasks and use reference costs to allocate two to four repetitions per arm; two is the minimum because the observed report estimates each task-arm sample variance. The export intentionally contains no local execution model. The Go commands require `--execution <model>:<effort>`, validate and execute the exact pasted or downloaded allocation with that configuration for both arms, and never optimize tasks or repetitions. The earlier locked nine-task suite is superseded.
...
- Campaign identity combines the exact website plan with the explicit local execution configuration. Baseline identity excludes Agent Layer treatment content and scheduler task concurrency. A changed effective instruction/skill bundle creates a new treatment version while compatible local baseline attempts remain reusable whether missing tasks execute sequentially or with bounded workers.
