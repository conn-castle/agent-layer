...
- Sync seeds `.grok-config/trusted_folders.toml` for the current repo root when that exact folder entry is absent. It requires `.grok-config` to be a real directory so repo-local trust seeding cannot follow a symlink into unrelated Grok state.
