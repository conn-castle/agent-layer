...
- Study reports contain only declared experiments and every selected task, including partial cells. Pairwise inference uses observed within-cell run variance with weighted calibration coefficients, Welch–Satterthwaite degrees of freedom, raw two-sided p-values, and Holm adjustment over available pairs. Skills experiments declare `required_dispatch_roles`; workflow-noncompliant completed runs keep their scores, costs, and artifacts, but any pair involving them is unavailable. One run is descriptive; two runs enable the estimator and three are preferable for stability.
