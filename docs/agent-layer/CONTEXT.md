...
- Two DeepSWE workflows coexist. The recurring internal eight-task Luna-low matrix uses `.agent-layer/tmp/deepswe-luna-low-065-selection-replicate-2.json`, exactly one repetition per task, and the compatible completed baseline in matrix `c9be59278c2d…`. Schema-v2 plan campaigns are a separate workflow with two to four repetitions per selected task. Do not infer the matrix protocol from the campaign CLI or replace the established eight-task run with a new campaign plan.
- The report-compatible runner for the recurring matrix is commit `075ca0d`. Build that runner into `.agent-layer/tmp` and execute it from the current repository root so the stored baseline identity remains reusable while the treatment bundle fingerprints the current instruction and skill templates. Later matrix-runner commits changed task-environment or arm identity and can report `0/8` cached baseline cells against otherwise valid August 3 evidence; the required preflight result is `8/8` baseline cached and `0/8` treatment cached.
- The website presents task-level evidence and a budgeted baseline proxy rather than exporting benchmark plans. For every task, the generated artifact stores a deterministic one-run Pearson-correlation distribution and an ordinary least-squares calibration from mean F2P to the fixed full DeepSWE configuration score. Highlighted tasks are combined with normalized inverse residual-variance weights. After a published model is selected, the browser makes 1,000 deterministic draws from its complete four-run cells to show descriptive 5th–95th score and price repeatability ranges; these are not treatment-significance thresholds. An optional comparison point reuses the primary model's exact task allocation, repetitions, and weights without changing the table. The Go commands require an explicit plan and `--execution <model>:<effort>`, then execute that allocation with the same configuration for both local arms.
...
- Campaign identity combines the exact website plan with the explicit local execution configuration. Baseline identity excludes Agent Layer treatment content, scheduler task concurrency, and provider-client version. A changed effective instruction/skill bundle creates a new treatment version while compatible local baseline attempts remain reusable whether missing tasks execute sequentially or with bounded workers. Provider-client versions are preserved per arm; version differences remain combinable and produce a report warning because they may confound score or cost changes.
