...
- Sync seeds `.grok-config/trusted_folders.toml` for the current repo root when that exact folder entry is absent. It creates `.grok-config` with owner-only `0700` permissions, tightens an existing real directory that grants group/other access, and rejects symlinks or non-directories so trust seeding and Grok credentials remain repo-local and private. `al grok`, Grok dispatch, and `al vscode` create or tighten the same home before the Grok CLI can mkdir it at `0755`.
