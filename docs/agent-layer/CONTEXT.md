...
- Two DeepSWE workflows coexist. The recurring internal eight-task Luna-low matrix uses `.agent-layer/tmp/deepswe-luna-low-065-selection-replicate-2.json`, exactly one repetition per task, and the compatible completed baseline in matrix `c9be59278c2d…`. The website-planned campaign is a separate newer workflow with two to four repetitions per selected task. Do not infer the matrix protocol from the campaign CLI or replace the established eight-task run with a new website plan.
