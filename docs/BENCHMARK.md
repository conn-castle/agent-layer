...
- When the checkpoint proves that provider execution completed cleanly and failure began in the verifier, a later identical run replays only the verifier from the retained `model.patch`; the original provider event remains the canonical cost, duration, and stream evidence, and its receipt records verifier-replay provenance. A replay failure remains replayable and never authorizes another provider call. Agent-phase failures are never eligible for verifier replay.
- Readiness prints task percentages. Study runs print preparation stages, task/arm preflight and cell percentages, cumulative observed cost, and the active environment/provider/verifier phase with its effective timeout. The verifier phase covers both its separate image-build allowance and test-run timeout, so the displayed value is their combined upper bound. During a silent long-running operation, the CLI prints one heartbeat after 60 seconds of inactivity and at most once per additional silent minute; heartbeats include phase elapsed time, and genuine progress resets the timer.
- Cancellation gives Pier two minutes to run its shielded teardown. On Linux, fallback cleanup repairs `/logs` ownership inside each attributed running container before removing that container, so verifier-owned files remain readable for sanitization and promotion.
