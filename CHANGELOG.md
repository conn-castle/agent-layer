...
- Shipped instruction templates were also tightened: several generic rules moved out of always-loaded rules, response-style guidance was made explicit, tool-routing now prefers local files/CLIs before MCP or web retrieval when local sources can answer, and relevant skills should be activated automatically from their descriptions.
...
- Upgrade planning now treats wizard-managed CLI catalog skills as managed files instead of unknown files, and unpinned dev/legacy upgrades can trigger the latest source-agnostic migration only when legacy Gemini config or missing source-agnostic defaults prove there is migration work to do.
