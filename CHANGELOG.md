...
- `al doctor` now prints a **context size summary** after its checks: estimated instruction tokens, skill catalog metadata size (token estimate plus its character budget), and MCP totals (enabled servers, total tools, total tool-schema tokens), each shown against its configured threshold. The summary always prints — even when values are under threshold and even with `noise_mode = "quiet"` or `al --quiet doctor` — because it is informational, not a warning. Thresholds left unset show `(no limit set)`; unreachable MCP servers are excluded from totals with a note.
