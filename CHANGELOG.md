...
### Added
- **Agentic status lines for Claude Code and Codex**, on by default. `al sync` seeds an editable `.agent-layer/claude-statusline.sh`, projects it to `.claude/claude-statusline.sh`, and wires `statusLine` into `.claude/settings.json`; for Codex it injects a managed `[tui].status_line` block into `.codex/config.toml` from an editable `.agent-layer/codex-statusline.toml` (skipped when you define `agent_specific.tui.status_line` yourself). The Claude line renders model, reasoning effort, context %, weekly usage limit, session, directory, git branch/dirty state, lines changed, session cost, and PR; it requires `jq` on `PATH` and degrades to a one-line hint when absent. Opt out per agent with `agents.claude.statusline = false` / `agents.codex.statusline = false` in `.agent-layer/config.toml`; the v0.11.0 migration seeds these defaults to `true` on upgrade. Both source files are seed-once and never overwritten.
