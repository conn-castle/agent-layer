...
### Added
- `al wizard` now offers five per-agent **feature disable toggles**, folded into the existing model step (each defaults to **No**, keeping the client's native behavior): disable Codex browser/computer-use (`features.browser_use`/`in_app_browser`/`computer_use`) and built-in apps (`features.apps`); disable Claude's IDE open-file reading (`env.CLAUDE_CODE_AUTO_CONNECT_IDE`), auto-memory (`autoMemoryEnabled`), claude.ai connectors (`env.ENABLE_CLAUDEAI_MCP_SERVERS`), and the AskUserQuestion tool (`permissions.deny` plus a `PreToolUse` hook). Each writes its `agent_specific` key only when you opt in, and reads back from existing config so re-running the wizard preserves your choice. The Codex "apps" prompt is reworded to the shared "Disable …?" form (its enabled-state storage is unchanged).
