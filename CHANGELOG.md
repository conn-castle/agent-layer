...
### Changed
- Gemini sync migrates from the deprecated `tools.allowed` field to the Policy Engine. `WriteGeminiSettings` no longer emits `tools.allowed` in `.gemini/settings.json` and instead writes a `policyPaths: [".gemini/policies"]` pointer; `WriteGeminiPolicies` generates `.gemini/policies/agent-layer.toml` with one `[[rule]]` block (`toolName = "run_shell_command"`, `commandPrefix`, `decision = "allow"`, `priority = 100`) per allowed command. Resolves the Gemini CLI deprecation warning ("--allowed-tools cli argument and tools.allowed in settings.json are deprecated and will be removed in 1.0").
