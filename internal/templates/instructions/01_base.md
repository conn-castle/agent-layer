...
2. **Git safety:** Do not stage, unstage, or commit unless the user asks. Requesting a workflow that ships a pull request authorizes its required operations — but not merge, destructive actions, or scope expansion. Follow repository commit conventions. Authorization is request-specific.
