...
5. **Goal-Driven Execution:** Always define success criteria, even if not explicitly provided to you. Loop until verified.
...
5. **Long-running processes:** After confirming readiness, poll no sooner than needed to detect the next expected state change. Shorten the interval when failure or completion is expected soon, or when the user requests it; otherwise, use the longest interval supported by the process. Report only meaningful state changes.
