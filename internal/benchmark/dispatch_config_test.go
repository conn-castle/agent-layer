...
func TestLoadTreatmentDispatchConfigPreservesExactRoleTargets(t *testing.T) {
	...
	data := []byte(`schema = "agent-layer-benchmark-dispatch-v2"
[[plan_reviewers]]
agent = "codex"
model = "gpt-5.6-terra"
...
func TestLoadTreatmentDispatchConfigRejectsObsoleteFixerRole(t *testing.T) {
	model, _, err := ParseModelSelection("sol:medium")
	if err != nil {
		t.Fatal(err)
	}
...
`)
	...
	if _, err := loadTreatmentDispatchConfig(path, model); err == nil {
		t.Fatal("obsolete fixer role was accepted")
	}
}
...
func TestTreatmentDispatchConfigRejectsAProviderUnavailableToCoordinator(t *testing.T) {
	...
	config.CodeReviewer = TreatmentDispatchTarget{Agent: "claude", Model: "claude-fable-5", ReasoningEffort: "high"}
