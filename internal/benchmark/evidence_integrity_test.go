...
func TestVerifierBuildFailureIsReportedWithItsBuildOutput(t *testing.T) {
	stage := t.TempDir()
	verifier := filepath.Join(stage, "jobs", "job", "verifier")
	if err := os.MkdirAll(verifier, 0o700); err != nil {
		t.Fatal(err)
	}
	events := strings.Join([]string{
		`{"Action":"build-output","Output":"Step 3/9 : RUN pip install -r requirements.txt\n"}`,
		`{"Action":"other","Output":"ignored\n"}`,
		`not json`,
		`{"Action":"build-output","Output":"\nERROR: could not find a version that satisfies numpy==99\n"}`,
	}, "\n")
	if err := os.WriteFile(filepath.Join(stage, "jobs", "job", "build-events.jsonl"), []byte(events), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(verifier, "test-stdout.txt"), []byte("[build failed]\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	// A verifier that never built scores zero for reasons that have nothing to
	// do with the model, so the report must be able to say why instead of
	// presenting the zero as a model result.
	if !failed {
		t.Fatal("verifier build failure was not detected")
	}
	if !strings.Contains(excerpt, "numpy==99") || strings.Contains(excerpt, "ignored") {
		t.Fatalf("build excerpt = %q", excerpt)
	}
}

func TestVerifierBuildSuccessReportsNoExcerpt(t *testing.T) {
	stage := t.TempDir()
	verifier := filepath.Join(stage, "jobs", "job", "verifier")
	if err := os.MkdirAll(verifier, 0o700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(verifier, "test-stdout.txt"), []byte("3 passed\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if failed || excerpt != "" {
		t.Fatalf("successful build reported failure %t with excerpt %q", failed, excerpt)
	}
}
