...
func readArmResults(stateDir string, tasks []benchmarkPlanTask, checksums map[string]string) (map[string][]AttemptResult, error) {
	...
	for _, task := range tasks {
		for attempt := 1; attempt <= task.RepetitionsPerArm; attempt++ {
			...
			if canonical, exists, canonicalErr := readCanonicalResult(path, data, result); canonicalErr != nil {
				return nil, fmt.Errorf("read canonical benchmark result %s repetition %d: %w", task.ID, attempt, canonicalErr)
			} else if exists {
				result = canonical
			} else {
				result, err = applyScoreCorrections(result, path)
				if err != nil {
					return nil, fmt.Errorf("correct benchmark result %s repetition %d: %w", task.ID, attempt, err)
				}
			}
