...
const (
	...
	DispatchReservationFlag              = "Handle or invocation ID from al dispatch reserve to launch at most once"
	DispatchReserveShort                 = "Reserve a named invocation for a later idempotent start"
	DispatchReserveLong                  = "Durably create a named invocation in the \"reserved\" state without launching anything, and write its handle and invocation ID as one JSON object.\n\nLaunch it with `al dispatch start --reservation <handle>`; repeating that start never launches a second agent. Unstarted reservations expire after dispatch.reservation_expiry_days (default 7)."
