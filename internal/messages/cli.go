...
const (
	...
	UpdateInstalledVersionInvalidFmt  = "installed CLI reported an invalid version %q: %w"
	UpdateInstalledVersionWarnFmt     = "Warning: could not determine installed CLI version: %v\n"
	UpdateCompleteFmt                 = "Agent Layer CLI update complete: %s -> %s. Repository pins are unchanged; run `al upgrade plan` in each initialized repository.\n"
	...
	DispatchReservationFlag              = "Immutable invocation ID from al dispatch reserve to launch at most once"
	DispatchReserveShort                 = "Reserve a named invocation for a later idempotent start"
	DispatchReserveLong                  = "Durably create a named invocation in the \"reserved\" state without launching anything, and write its handle and invocation ID as one JSON object.\n\nLaunch it with `al dispatch start --reservation <invocation-id>`; repeating that start never launches a second agent. Unstarted reservations expire after dispatch.reservation_expiry_days (default 7)."
