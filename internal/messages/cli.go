...
const (
	...
	DispatchShort                         = "Run a supported headless agent with al dispatch"
	DispatchLong                          = "Run one supported headless target agent for a focused second-agent task.\n\nPrompt text can be passed as arguments or on stdin. Use --skill to invoke a portable Agent Layer skill in the target, with or without additional prompt text. Target answer text is written to stdout; Agent Layer status, selected-target notices, compact progress, and errors are written to stderr.\n\n`al dispatch` is meant for real agent work and can run for tens of minutes; around an hour is possible for complex tasks. Agent Layer does not impose a short global timeout, though target CLIs may enforce their own limits. Streaming output is intentional so progress can be watched while long runs continue.\n\nRun `al dispatch options` before composing a call to inspect available targets, caller detection, random-selection behavior, and per-target model or reasoning-effort override support. Use a concrete --agent value for reproducible scripts; random target selection is intended for interactive second opinions. If --agent is omitted, Agent Layer must know the caller from its launch environment or the command fails.\n\n`al dispatch` supports depth 1 only. A dispatched target cannot call `al dispatch` again while AL_DISPATCH_ACTIVE=1 is set. The target runs with its own Agent Layer permissions and approvals; `al dispatch` is not a permission-containment boundary."
	...
	DispatchOptionsShort                  = "List al dispatch targets and override metadata"
	DispatchOptionsLong                   = "List the current `al dispatch` target contract.\n\nText output is for humans. Use --json for the stable v1 machine-readable shape, including caller detection, random-selection pool, per-target enablement and install status, dispatch capability, streaming behavior, model override metadata, reasoning-effort override metadata, and unavailable reasons."
	DispatchOptionsJSONFlag               = "Emit stable machine-readable JSON"
	DispatchTargetImplicitFmt             = "`al dispatch` target: %s (from agents.%s.dispatch.default_agent)"
	DispatchTargetRandomFmt               = "`al dispatch` target: %s (random selection)"
	DispatchNestedActive                  = "`al dispatch` is already active; nested `al dispatch` is not supported"
	...
	DispatchDisabledTargetFmt             = "`al dispatch` target %s is disabled in config"
	DispatchMissingBinaryFmt              = "`al dispatch` target %s requires `%s` on PATH"
	...
	DispatchOptionsHeader                 = "al dispatch options"
	...
	DispatchInternalErrorFmt              = "`al dispatch` internal error: %v"
