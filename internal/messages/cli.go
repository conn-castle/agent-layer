...
const (
	...
	DispatchShort                         = "Run one fresh, final-answer-only headless agent turn"
	DispatchLong                          = "Run one fresh supported headless agent turn. Prompt text can be passed as arguments or on stdin; --skill invokes a portable Agent Layer skill. Standard output contains only the successfully completed assistant answer. Standard error contains one compact identity line and actionable failures.\n\nUse `al dispatch resume <name> <prompt>` only to continue an explicitly named conversation. `inspect`, `list`, and `delete` operate only on Agent Layer-owned mappings and evidence. Agent Dispatch never infers a prior conversation from a target, prompt, or artifact path.\n\nRun `al dispatch options` to inspect exact fresh, resume, and inspection capabilities for the installed provider versions. Dispatch defaults to max depth 3; a dispatched target can make two nested calls before the depth guard blocks another one."
	...
	DispatchOptionsLong                   = "List current Agent Dispatch capability facts.\n\nText output is for humans. Use --json for the stable v0.13 machine-readable shape, including installed and supported provider versions plus separate fresh, resume, and inspection capabilities."
	...
	DispatchEmptyRandomOverridePoolFmt    = "no agents eligible for `al dispatch --agent random` to support all requested overrides (%s); remove unsupported overrides or select an explicit compatible agent"
