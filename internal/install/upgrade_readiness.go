...
// MCP config file (.mcp.json or .gemini/settings.json). These files are wholly
// generated by `al sync`, so the presence of the "mcpServers" key is sufficient
// evidence. This matches both current output (no prompt server) and legacy output
// (which included an "agent-layer" server entry with "mcp-prompts" args).
func hasAgentLayerMCPSignature(data []byte) (bool, error) {
	return strings.Contains(string(data), "\"mcpServers\""), nil
}
