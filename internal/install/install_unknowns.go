...
func (inst *installer) buildKnownPaths() (map[string]struct{}, error) {
	...
	// Machine-managed skill import state. The lockfile and the entire imported
	// tree are Agent Layer-owned, but their contents are not upgrade templates:
	// they are registered as known so `al upgrade` never offers to delete a
	// user's imported skills or their recorded upstream state.
	add(filepath.Join(root, ".agent-layer", config.SkillsLockFileName))
	...
	// Imported skill trees are registered recursively so every nested resource
	// file stays known, not just the tier root.
