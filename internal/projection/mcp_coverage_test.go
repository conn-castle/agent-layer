...
func TestEnabledServerIDs_AppliesToClient(t *testing.T) {
	...
	servers := []config.MCPServer{
		{ID: "antigravity-only", Enabled: &enabled, Clients: []string{"antigravity"}},
	...
	// Should include "antigravity-only" and "all". "claude-only" excluded.
	if len(ids) != 2 {
		t.Fatalf("expected 2 ids, got %d", len(ids))
	}
	if ids[0] != "all" || ids[1] != "antigravity-only" {
...
func TestEnabledServerIDs_NilEnabled(t *testing.T) {
	...
	servers := []config.MCPServer{
		{ID: "nil-enabled", Enabled: nil, Clients: []string{"antigravity"}},
	}
	ids := EnabledServerIDs(servers, "antigravity")
	if len(ids) != 0 {
...
func TestResolveMCPServers_NilEnabled(t *testing.T) {
	...
	resolved, err := ResolveMCPServers(servers, map[string]string{}, "antigravity", nil)
	if err != nil {
