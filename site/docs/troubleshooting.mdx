...
:::caution
`.codex/config.toml` and `.agy/antigravity-cli/settings.json` contain shared native state patched by `al sync`. Historical `.muse-config/` and `.muse-data/` may also contain native settings, credentials, and non-reproducible sessions. Treat these paths as private and keep them gitignored. Agent Layer-managed MCP output remains safe to regenerate.
:::
