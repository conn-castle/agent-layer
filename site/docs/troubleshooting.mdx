...
When `agents.codex.local_config_dir = true`, `al codex`, Codex dispatch, and `al vscode` set `CODEX_HOME=<repo>/.codex`. The generated `.codex/config.toml` includes config projected from `.agent-layer/config.toml`, including the current repo trust stanza and MCP servers.
