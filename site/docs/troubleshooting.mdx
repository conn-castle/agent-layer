...
Agent Layer does not install Antigravity, Claude, Codex, Copilot CLI, Grok, Muse, or VS Code. Install the target client and ensure it is on your `PATH`.
...
Generated client outputs should stay gitignored. If you see files like `.agents/skills/`, `.agy/`, `.antigravitycli/`, `.claude/settings.json`, `.claude/skills/`, `.claude/rules/agent-layer.md`, `.codex/`, `.copilot/`, `.grok/`, `.grok-config/`, `.muse-config/`, `.muse-data/`, `.vscode/mcp.json`, `.vscode/settings.json`, `.mcp.json`, `.github/copilot-instructions.md`, or `AGENTS.md` showing up in `git status`, it usually means one of:
...
Muse uses `.muse-config/` and `.muse-data/` as repo-local XDG roots for `al muse`, model discovery, and dispatch. `HOME` is unchanged, but child tools inherit `XDG_CONFIG_HOME` and `XDG_DATA_HOME`; preserve explicit tool-specific overrides such as `GH_CONFIG_DIR` when needed. A new repository may require Muse login even when the global Muse installation is authenticated.
...
:::caution
`.codex/config.toml`, `.agy/antigravity-cli/settings.json`, and `.muse-config/muse/settings.json` are shared state patched by `al sync`, not disposable generated files. They can contain native settings outside Agent Layer's managed paths; `.muse-data/` can also contain non-reproducible sessions. Treat these paths as private and keep them gitignored. Agent Layer-managed MCP output remains safe to regenerate.
:::
...
Install the target client CLI (Antigravity `agy`, Claude, Codex, Copilot CLI, Grok, Muse, or VS Code) and ensure it is available on your `PATH`.
...
## Muse model suggestions and duplicate rules warnings
...
Run `al sync` to migrate the generated Claude instruction shim to `.claude/rules/agent-layer.md`. This keeps Claude's instructions while avoiding Muse's warning that `.claude/CLAUDE.md` is ignored beside `AGENTS.md`. Sync preserves hand-written files; if the warning names one of those, merge any needed guidance into `.agent-layer/instructions/` before removing the redundant file yourself.

Grok may print “not authenticated” alongside a valid model catalog. Agent Layer accepts that catalog for model selection; authentication is still required when you run the model. If Grok returns no catalog, discovery reports the failure.
