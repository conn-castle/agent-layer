...
| `al skills reset <name>` | Permanently discard one imported skill's local edits and accept its current configured upstream version. |
...
- Reads `.agent-layer/instructions/`, both `.agent-layer/skills/` and `.agent-layer/imported-skills/`, `.agent-layer/commands.allow`, and `.agent-layer/.env`
...
User-managed `.agent-layer/skills/` and Git-managed `.agent-layer/imported-skills/` use directory format:
...
at the branch's current commit; ordinary pull leaves that new pin fixed.
