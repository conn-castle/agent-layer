...
# How long an `al dispatch reserve` reservation stays startable before it is
# retired without launching. Optional; defaults to 7.
...
| `[dispatch]` | Agent Dispatch nesting depth (`max_depth`), session retention (`session_retention_days`), reservation expiry (`reservation_expiry_days`), and MCP timeouts (`mcp_wait_timeout_minutes`, `mcp_tool_timeout_minutes`) |
...
Muse is optional and disabled when `[agents.muse]` is absent. Agent Layer does not provide Muse storage isolation. Interactive launches, dispatch, and model discovery leave `HOME`, `XDG_CONFIG_HOME`, and `XDG_DATA_HOME` unchanged, so Muse uses its native user settings, authentication, and session storage. Agent Layer does not write project configuration into the user's global Muse settings.
...
Project MCP uses native Muse startup behavior. The project schema does not accept the user-settings `mode` field, so Agent Layer cannot mark the dispatch server required or user servers optional. A failed project server can remain unavailable while a model turn completes. The built-in dispatch server retains its tool timeout. Run sync after editing project MCP configuration; `--no-sync` uses the existing file.
...
and allows ten seconds per lookup. Wizard starts queries before its first configuration screen; disabled Muse waits until you enable it and open its model picker, waits only when
...
| --- | --- |
| Claude Code (`.mcp.json`) | `${AL_TOKEN}`; entries shared with enabled Muse use resolved values in a private `0600` file |
...
| Muse (`.mcp.json`, shared with Claude) | resolved literal value in a private `0600` file |
...
- `dispatch.reservation_expiry_days` must be a positive integer representable as a duration when set
...
| `al dispatch reserve` | Create a named invocation that launches nothing until `al dispatch start --reservation` starts it once. |
...
Generated outputs are safe to delete, and `al sync` will recreate them — except the shared-state files it patches in place, `.codex/config.toml`, `.agy/antigravity-cli/settings.json`, and `.muse/hooks.json`. Retain `.muse/agent-layer-policy.json` until disabling Muse and syncing so native grants can be retired. Preserve historical Muse settings and sessions under `.muse-config/` and `.muse-data/` as well. Keep these gitignored but do not treat them as disposable: native values in the settings files and Muse sessions in the data root may not be reproducible. Managed MCP output remains safe to regenerate.
