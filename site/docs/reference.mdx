...
`al doctor` also prints a context size summary (instruction tokens, skill catalog metadata size, and MCP totals against their thresholds). It is informational, not a warning, so it always prints — even under `noise_mode = "quiet"` or `al --quiet doctor`. Thresholds left unset show `(no limit set)`.
