...
standard output. `inspect` reports read-only transport facts, while `delete`
...
initial exact compatibility set is Claude Code 2.1.207, Codex CLI 0.144.1, and
Antigravity 1.1.1. See `docs/AGENT-DISPATCH.md` in the repository for the full
v0.13 contract.
...
- `69` — target unavailable (missing target binary, unsupported provider version, no eligible target in the random-selection pool)
