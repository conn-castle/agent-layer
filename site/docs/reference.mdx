...
- Before deletion prompts, offers to add unknown non-tmp paths to the repo-local `.agent-layer/upgrade-keep-list`; selected paths are omitted from future upgrade prompts and plans
...
Interactive upgrades with unknown non-tmp paths first ask whether you want to add any of them to `.agent-layer/upgrade-keep-list`. If accepted, a checkbox list lets you select intentional local files or directories. The ordinary deletion flow then continues with the remaining unknown paths. When every detected path is already kept, upgrade shows neither prompt.
...
A directory entry keeps its entire subtree. Entries are exact paths—wildcard characters have no special meaning—and paths outside `.agent-layer/` or `docs/agent-layer/` are rejected. Missing entries remain in the file until you edit it; Agent Layer does not prune them. Kept paths are omitted from `al upgrade plan`, interactive deletion prompts, and `--apply-deletions`. The list does not affect managed-template overwrite decisions. `.agent-layer/tmp/` remains outside this mechanism and retains its separate protected cleanup flow.
