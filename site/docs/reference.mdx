...
Only `add`, `pull`, and `push` contact remote repositories. `status`, `al sync`,
...
skill it was hiding, again at the block's locked commit. A block left without a
