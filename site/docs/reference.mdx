...
| --- | --- | --- |
| `repository` | yes | Source Git repository, reached through your existing Git authentication. |
...
| `push_repository` | no | Destination repository for upstream writes. Defaults to `repository`; set it to contribute through a fork. |
...
| `al skills add <repository> <selector>...` | Import skills from a Git repository and project them. |
| `al skills remove <repository> <selector>` | Remove one configured import selector and recompute the desired set. |
...
| `al skills push` | Publish local imported-skill changes to configured destinations. |
...
upstream state lives in `.agent-layer/skills.lock.json`. Both are ordinary
files: whether you commit them is your project's choice, and Agent Layer never
stages or commits your repository.

`add`, `pull`, and `push` always contact remote repositories, and `remove`
contacts one whenever the block keeps a selector, because it re-resolves the
remaining desired set at the block's locked commit. Removing a block's last
positive selector is the only `remove` that stays local. `status`, `al sync`,
and agent launch are always local.
...
result. For an existing block it resolves the selector change at that block's
locked source commit without advancing it. Configuration, imported skills, and
lock state change together, so a failure leaves your prior state intact. It
never searches, recommends, or previews skills.
...
skill it was hiding, again at the block's locked commit; resolving that
remaining desired set opens the source, so `remove` contacts the remote unless
it is removing the block's last positive selector. A block left without a
positive selector is removed.
...
to run `al skills pull` if it moved.
...
identifies the block — so the lockfile stays shareable and identical across
machines. The value is resolved only when a Git command runs, and is redacted
...
**Ownership.** `.agent-layer/imported-skills/` is fully managed. A directory
with no lock entry, or one name owned by both `.agent-layer/skills/` and
`.agent-layer/imported-skills/`, is an actionable error rather than a silent
shadow.
