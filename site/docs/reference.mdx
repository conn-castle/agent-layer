...
Agent Layer is explicit by design. It prefers failing fast over guessing, treats generated files as disposable outputs except for documented shared native state, and patches only owned paths in shared client settings. Use this page when you need to know exactly what will change on disk.
...
Shared project instructions for Grok live in root `AGENTS.md`. The Claude instruction shim is `.claude/rules/agent-layer.md`. When Grok is enabled, sync sets `[compat.claude] agents = false` in repo-local `.grok-config/config.toml`, and Grok launch, dispatch, and `al vscode` set `GROK_CLAUDE_AGENTS_ENABLED=false`, so Grok skips legacy Claude-named instruction files. A leftover hand-authored root `CLAUDE.md` can still dual-load in Grok. Grok also discovers Claude-compatible hooks; Agent Layer's Claude chime handler silently ignores Grok's camelCase hook envelope so the dedicated Grok handler is the only one that chimes.

Claude instructions are generated in `.claude/rules/agent-layer.md`, which Claude loads automatically and Muse does not treat as a competing root rules file. Sync removes the old `.claude/CLAUDE.md` when it is empty or carries Agent Layer's generated-file header. Hand-written content is preserved.

**Muse Code 1.3.0**

Muse is optional and disabled when `[agents.muse]` is absent. `al muse` and Muse dispatch set `XDG_CONFIG_HOME=<repo>/.muse-config` and `XDG_DATA_HOME=<repo>/.muse-data` without changing `HOME`; Muse child tools inherit those XDG values, which can change where XDG-aware developer tools look for state. Existing `GH_CONFIG_DIR` and `GIT_CONFIG_GLOBAL` values are not changed. The isolated Muse home may require a separate login.

Sync preserves unrelated native settings while replacing the canonical `mcpServers` member in private `0600` settings. An existing file must declare supported numeric `schema_version = 1`; missing, malformed, or newer schemas fail explicitly instead of being rewritten. Stdio and Streamable HTTP are supported; explicit or default SSE is rejected instead of rewritten. Runtime admission of the projected stdio shape is verified on Muse 1.3.0. The built-in Agent Dispatch server is required and user servers are optional: if that required server cannot start, Muse rejects the whole run before the model turn, including ordinary interactive `al muse` runs.

Muse also reads `<workspace>/.mcp.json`, which Claude and Claude VS Code use. To preserve each client's server selection, Agent Layer writes `enabled: false` entries for that project's server names in Muse's settings, then projects Muse-selected servers with a `muse-` prefix (extended if necessary to avoid a name collision). Native Muse 1.3.0 probes verify that the disabled project servers do not start while the Muse entries initialize normally. Claude's configuration, placeholders, and tools remain unchanged. Run sync after editing project MCP configuration; `--no-sync` deliberately uses the previously generated settings.

Muse has no verified native schema for Agent Layer's selective command or MCP grants. Non-YOLO runs keep native approvals and sandboxing, pass `--approval-judge off`, and never receive `--disable-approval`; only explicit `approvals.mode = "yolo"` receives `--yolo`. Headless dispatch remains available under every approval mode. A read-only `muse serve` observer checks the dispatch session through Muse's supported `approval/listPending` method; when an action requires human approval, Agent Layer fails the dispatch and verifies provider termination without an inactivity deadline or broader permissions. Muse's separate `request_user_input` tool is auto-cancelled in headless dispatch with `--user-input-auto-resolve`. Completion chime and statusline projection are unsupported because no sufficiently precise native completion-hook/statusline contract was established.
...
Codex, Grok, Muse, Antigravity, and Copilot CLI. Discovery uses the project's normal provider
...
not duplicated as separate choices. Discovery may create the normal repo-local `.agy`, `.grok-config`, `.muse-config`, and `.muse-data` directories
...
- Muse supports `model` through a bounded, non-inference MSP `model/list` query and `reasoning_effort` (`none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`). Muse 1.3.0 can return an empty native catalog; Agent Layer explains that model suggestions are unavailable and lets you choose the client default or enter a custom model. Discovery never substitutes a static model list.
...
enabled Codex, Claude, Antigravity, VS Code, Copilot CLI, Grok, and Muse client. It serves
...
- `clients` to restrict which clients receive a server (valid values: `antigravity`, `claude`, `vscode`, `codex`, `copilot`, `grok`, `muse`; note that `claude_vscode` uses the `claude` client)
...
| Muse (`.muse-config/muse/settings.json`) | resolved literal value in a private `0600` file |
...
- `.muse-config/muse/settings.json` plus `.muse-data/` runtime state when Muse is enabled
- `.vscode/mcp.json` and a managed block in `.vscode/settings.json`
- `.github/copilot-instructions.md`
- `AGENTS.md`, `.claude/rules/agent-layer.md`
...
Generated outputs are safe to delete, and `al sync` will recreate them — except the shared-state files it patches in place, `.codex/config.toml`, `.agy/antigravity-cli/settings.json`, and `.muse-config/muse/settings.json`, plus Muse runtime state under `.muse-data/`. Keep these gitignored but do not treat them as disposable: native values in the settings files and Muse sessions in the data root may not be reproducible. Managed MCP output remains safe to regenerate.
...
Valid dispatch targets are `codex`, `claude`, `antigravity`, `grok`, and `muse`; VS Code
