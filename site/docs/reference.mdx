...
Current JSON contract (additive):

- top-level `schema_version`
- per diff entry:
  - `ownership` (legacy display label; backward compatible)
  - `ownership_state` (`upstream_template_delta`, `local_customization`, `mixed_upstream_and_local`, `unknown_no_baseline`)
  - `ownership_confidence` (`high`, `medium`, `low`, omitted for unknown)
  - `ownership_baseline_source`
  - `ownership_reason_codes`

Prefer `ownership_state` for new automation; keep `ownership` for legacy consumers.
