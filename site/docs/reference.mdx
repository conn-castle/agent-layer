...
Agent-specific passthrough keys in `agents.codex.agent_specific` or `agents.claude.agent_specific` override Agent Layer-managed keys when they collide. Agent Layer emits a warning on every sync if you override managed keys. Claude `permissions.deny` is the one exception: it is deep-merged additively into `.claude/settings.json` and does not warn.
