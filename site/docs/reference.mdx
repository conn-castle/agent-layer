...
| `al upgrade` | Apply template-managed updates and update the repo pin (prompts unless `--force`; line-level diff previews shown by default, configurable with `--diff-lines`). |
| `al upgrade plan` | Show a dry-run, categorized upgrade plan with ownership labels and line-level diff previews (configurable with `--diff-lines`). |
...
- Prompts before overwriting managed template files (use `--force` to skip prompts)
- Shows line-level diff previews before overwrite decisions by default
- Deletes unknown files under `.agent-layer` only if you explicitly approve each deletion (or use `--force`)
...
- Readiness checks (for example unrecognized config keys, potentially stale VS Code `--no-sync` outputs, floating dependency specs, and stale disabled-agent artifacts)

Each diff entry includes an ownership label:

- `upstream template delta`
- `local customization`
- `mixed upstream and local`
- `unknown no baseline`

`unknown no baseline` means Agent Layer cannot prove ownership safely for that file from baseline evidence. In text output, `al upgrade plan` includes an **Ownership warnings** block with actionable guidance.
...
`al upgrade plan --json` is available as optional diagnostic output. Its field-level shape is not a stable public schema contract and may change between releases.
