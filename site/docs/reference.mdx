...
Generated outputs are safe to delete, and `al sync` will recreate them. Keep `.agy/antigravity-cli/settings.json` gitignored but do not treat it as disposable: native Antigravity values, including workspace approval or trust, are preserved there. Managed MCP output remains safe to regenerate.
