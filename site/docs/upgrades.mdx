...
| --- | --- | --- | --- | --- |
| `v0.8.2` | `0.8.x`, `0.7.x` (via migration chaining) | `safe auto`: Migration manifests are now chained during multi-version upgrades. Users jumping from 0.8.0 directly to 0.8.2 will now receive the `agents.claude-vscode.enabled` config prompt that was introduced in v0.8.1. Config resilience improvements ensure `al wizard`, `al doctor`, and `al upgrade` always work even on broken configs. | No manual action required; run `al upgrade plan` then `al upgrade`. | Migration chaining loads all intermediate manifests between source and target versions. When source version is unknown, only the target manifest is loaded (backward compatible). |
