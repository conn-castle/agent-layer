...
| --- | --- | --- | --- | --- |
| `v0.13.0` | `0.10.x`, `0.11.0`, `0.12.0`, `0.12.1`, `0.12.2`, `0.12.3` | `breaking/manual`: Agent Dispatch is final-answer-only and fresh by default. The v1 streaming and `dispatch_capable` options contract is removed; explicit `resume`, `inspect`, `list`, and `delete` replace implicit lifecycle assumptions. `needs review`: workflow root skills are renamed/retired, `fix-issues` is absorbed by the mode-driven `auto-skill-loop`, and the version-binary handoff package is unambiguously named `internal/versiondispatch`. | Update scripts and automations that parse dispatch progress, provider stderr, raw events, or v1 `dispatch_capable` fields. Consume only final stdout, save the emitted friendly name when later continuation is intended, and use `al dispatch resume <name> ...` rather than inferring a conversation from a target or prompt. Back up any customized retired or renamed skill directory, then run `al upgrade plan` and `al upgrade`; review managed workflow skill changes and approve removal only for retired directories you no longer need. | Migration manifest renames whole tracked skill directories and carries bundled resources through those renames. Retired directories, including `fix-issues`, use the ownership-aware reviewed removal path rather than an unconditional directory deletion. The release also adds the 0.13.0 ownership baseline. Provider support is exact-version gated: Claude Code 2.1.207, Codex CLI 0.144.1, and Antigravity 1.1.1. Existing provider conversations are not deleted or imported. |
