...
| --- | --- | --- | --- | --- |
| `v0.11.2` | `0.10.x`, `0.11.x` | `needs review`: Makes repo-local Codex home opt-in via `agents.codex.local_config_dir`. Agent Layer still writes repo-local `.codex/config.toml`, including project trust and MCP config, but `al codex`, Codex dispatch, and `al vscode` set `CODEX_HOME=<repo>/.codex` only when the new setting is `true`. | Run `al upgrade plan` then `al upgrade`. The upgrade prompts for `agents.codex.local_config_dir`; choose `true` to keep the previous repo-local Codex auth/session/log behavior, or choose `false` to use Codex's normal global/project config layering. | Migration manifest sets `agents.codex.local_config_dir = false` by default for non-interactive upgrades. When false or absent, Agent Layer preserves any inherited `CODEX_HOME` instead of setting or clearing it. |
| `v0.11.1` | `0.10.x`, `0.11.0` | `safe auto`: Adds Antigravity model selection to `al wizard`, `al dispatch --model`, and `al dispatch options`; Antigravity reasoning level is encoded in the agy model display string rather than a separate reasoning-effort field. The wizard also adds a Git tracking step that rewrites the managed `.agent-layer/gitignore.block` source for `.agent-layer/` and `docs/agent-layer/` tracking choices. | No manual action required; run `al upgrade plan` then `al upgrade`. Re-run `al wizard` if you want to set an Antigravity model or change Agent Layer folder tracking. | Migration manifest moves `agents.antigravity.agent_specific.model` to `agents.antigravity.model` before validation rejects that passthrough key. It preserves the v0.11.0 supported range so unpinned legacy upgrades still discover v0.10.2 Gemini-to-Antigravity source-agnostic migrations and v0.11.0 statusline defaults through the chained planner. |
