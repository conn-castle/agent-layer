...
| --- | --- | --- | --- | --- |
| `unreleased` | `0.7.x` | `breaking/manual`: `al init` is now one-time scaffolding and no longer supports upgrade/repair; `al init --overwrite` and `al init --force` removed. | Replace any automation that runs `al init --force` with `al upgrade --force`. For interactive flows, run `al upgrade plan` then `al upgrade`. | `al upgrade` never overwrites `.agent-layer/.env` or `.agent-layer/config.toml`. Root `.gitignore` managed block is still updated automatically and is excluded from upgrade plans. |
