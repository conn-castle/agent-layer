...
| --- | --- | --- | --- | --- |
| `v0.8.8` | `0.8.x`, `0.7.x` (via migration chaining) | `safe auto`: Wizard UX hardening (`Esc` back-navigation with first-step exit confirmation and state rollback), explicit wizard config ordering policy, profile warning when replacing TOML-corrupt config, Claude reasoning-effort support (`low|medium|high`) with fail-loud model validation, explicit Gemini reasoning-effort unsupported-path errors, multiline TOML patch maintainability refactor, and upgrade diff readability improvements with interactive colorized hunks plus plain fallback. | No manual action required; run `al upgrade plan` then `al upgrade`. Optionally set `[agents.claude] reasoning_effort = \"low|medium|high\"` when using supported Claude models. | No migration operations required. New required-field migration guardrail test enforces future `config_set_default` parity for required fields introduced after baseline `0.8.1`. |
| `v0.8.7` | `0.8.x`, `0.7.x` (via migration chaining) | `safe auto`: Added quiet-mode support across runtime surfaces: `warnings.noise_mode = "quiet"` plus one-off `--quiet`/`-q` client flags. Quiet mode suppresses Agent Layer informational output (warnings/update checks/dispatch banners), while `al doctor` always prints warnings. Includes dispatch and sync-path hardening plus new quiet e2e coverage. | No manual action required; run `al upgrade plan` then `al upgrade`. Optionally set `warnings.noise_mode = "quiet"` or use `al <client> --quiet` for one-off runs. | No config migration operations required. Existing configs remain valid; `noise_mode` now accepts `quiet` in addition to existing values. |
