...
| --- | --- | --- | --- | --- |
| `v0.11.0` | `0.10.x` | `safe auto`: Adds default-on provider statuslines for Claude Code and Codex. Claude sync seeds editable `.agent-layer/claude-statusline.sh`, projects it to `.claude/claude-statusline.sh`, and wires `statusLine` in `.claude/settings.json`. Codex sync seeds editable `.agent-layer/codex-statusline.toml` and merges its `[tui].status_line` list into `.codex/config.toml` unless `agents.codex.agent_specific.tui.status_line` is set. | Run `al upgrade plan` then `al upgrade`. The upgrade prompts for `agents.claude.statusline` and `agents.codex.statusline`; accept `true` to keep the new defaults or choose `false` to opt out. Claude's default statusline script requires `jq` on `PATH` at runtime. | Migration manifest includes `config_set_default` operations for `agents.claude.statusline = true` and `agents.codex.statusline = true`. The editable source files are user-owned and seeded once; future template improvements do not overwrite existing edits. For unpinned legacy upgrades, the manifest also carries forward v0.10.2 Gemini->Antigravity source-agnostic operations until the known unpinned migration-chain issue is resolved. |
