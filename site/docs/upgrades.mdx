...
| --- | --- | --- | --- | --- |
| `v0.8.6` | `0.8.x`, `0.7.x` (via migration chaining) | `safe auto`: Documentation-only follow-up release. Clarifies VS Code MCP header projection behavior (`${env:VAR}` placeholders, no auto-generated `inputs` block) and clarifies `make dev` scope (`test-release` included). | No manual action required; run `al upgrade plan` then `al upgrade`. | No migration operations are required. |
