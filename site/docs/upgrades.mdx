...
| --- | --- | --- | --- | --- |
| `v0.9.0` | `0.8.x`, `0.7.x` (via migration chaining) | `safe auto`: Migration manifest support for `v0.9.0` includes canonical config-key migration (`agents.claude-vscode.enabled` -> `agents.claude_vscode.enabled`), required-field defaulting, legacy `.agent-layer/slash-commands/` -> `.agent-layer/skills/` rename, and embedded skill template migration to directory format. `needs review`: [agentskills.io specification](https://agentskills.io/specification) alignment updates validation/normalization behavior (YAML frontmatter parsing, Unicode NFKC-aware name matching, rune-based limits, canonical `SKILL.md` naming). | Run `al upgrade plan` then `al upgrade`. If you have custom automation/scripts that still reference `.agent-layer/slash-commands/`, update them to `.agent-layer/skills/`. If you maintain directory-format skills, use canonical `SKILL.md` naming (lowercase `skill.md` remains a compatibility fallback but triggers a warning). | Migration manifest includes one `config_rename_key`, one `config_set_default`, and directory/file rename operations for bundled skills. Existing custom skills continue to load from both flat (`<name>.md`) and directory (`<name>/SKILL.md`) formats; strict validation now catches malformed names/metadata earlier. |
