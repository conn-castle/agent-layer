...
| --- | --- | --- | --- | --- |
| `v0.13.0` | `0.10.x`, `0.11.0`, `0.12.0`, `0.12.1`, `0.12.2`, `0.12.3` | `breaking/manual`: Agent Dispatch is final-answer-only and fresh by default. The v1 streaming and `dispatch_capable` options contract is removed; explicit `resume`, `inspect`, `list`, and `delete` replace implicit lifecycle assumptions. `needs review`: workflow root skills are renamed/retired and the version-binary handoff package is unambiguously named `internal/versiondispatch`. | Update scripts and automations that parse dispatch progress, provider stderr, raw events, or v1 `dispatch_capable` fields. Consume only final stdout, save the emitted friendly name when later continuation is intended, and use `al dispatch resume <name> ...` rather than inferring a conversation from a target or prompt. Run `al upgrade plan` then `al upgrade`; review managed workflow skill changes if you customized them. | Migration manifest renames the tracked workflow roots, removes only generated retired root-skill files, and adds the immutable 0.13.0 ownership baseline. Provider support is exact-version gated: Claude Code 2.1.207, Codex CLI 0.144.1, and Antigravity 1.1.1. Existing provider conversations are not deleted or imported. |
