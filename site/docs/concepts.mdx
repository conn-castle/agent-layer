...
- `.muse-config/muse/settings.json` (managed MCP settings merged with native settings)
- `.grok/config.toml`
...
Generated outputs are safe to delete and regenerate, except for shared native state. `al sync` patches `.codex/config.toml`, `.agy/antigravity-cli/settings.json`, and `.muse-config/muse/settings.json` in place while preserving native values it does not manage. Muse sessions and other runtime state under `.muse-data/` are also not reproducible. Keep these paths gitignored, but preserve them during cleanup.
...
Muse does not project selective command or MCP grants. All non-YOLO modes retain native approvals and sandboxing with `--approval-judge off`; headless dispatch fails if human approval is required.
