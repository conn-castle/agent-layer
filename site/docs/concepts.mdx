...
**Grok** home isolation is always on. `al grok`, Grok dispatch, and `al vscode` set `GROK_HOME=<repo>/.grok-config` so user auth stays out of generated project `.grok/`. Auth and sessions are per repository, so the first Grok launch in a repo may require login even when global Grok is authenticated. Sync seeds `.grok-config/trusted_folders.toml` for the current repo when that exact folder entry is absent.
