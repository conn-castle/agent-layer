...
Skills are synced natively to each client's skill directory with full subdirectory support (`scripts/`, `references/`, `assets/`). No MCP server is involved; `al sync` copies skill sources directly into each enabled client's native format.
