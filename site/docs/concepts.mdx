...
Generated outputs are safe to delete and regenerate. `.agy/antigravity-cli/settings.json` is the exception: it can also hold native Antigravity workspace approval, trust, and other settings. Keep it gitignored, but preserve it during cleanup; Agent Layer patches only its managed model, `permissions.allow`, and `agent_specific` paths, and never deletes native values it did not set.
