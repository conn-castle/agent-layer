...
- **Client projection** — User-managed `.agent-layer/skills/` and Git-managed `.agent-layer/imported-skills/` are canonical. `al sync` projects their exact trees to `.agents/skills/` for shared-skill clients and `.claude/skills/` for Claude Code.
...
Imported skills land in `.agent-layer/imported-skills/<skill-name>/` and are
...
you committed them in your own project, and it never pulls first.
