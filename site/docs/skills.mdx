...
- **A workflow-bundle library of 27 skills** covering planning, implementation, review, debugging, PR lifecycle, auditing, and codebase improvement — available when the workflow bundle is installed through the wizard and refreshed through upgrade flows for repos that already have it.
...
The Agent Layer workflow bundle includes 27 skills. They are organized here by how you typically use them. Agent Layer also ships optional CLI catalog skills for installed tools; those are listed separately below.
...
| `ship-pr` | Full PR lifecycle: audits uncommitted changes, commits, pushes, creates the PR, monitors CI (fixing failures), waits for review comments, addresses every one, and confirms CI passes. Delegates to `clean-and-fix-code`, `fix-ci`, and `address-pr-comments`. |
...
| `resolve-findings` | Resolves a review report by verifying each finding, assigning verdicts, grouping accepted fixes, coordinating bounded fix work, and recording why findings were fixed, rejected, deferred, or already resolved. |
...
| `loop-clean-and-fix` | Repeats `clean-and-fix-code` on the current uncommitted working tree while a cleanup round resolves Critical or High findings. |
...
| `run-and-fix-checks` | Runs the full documented check lane, including all tests and CI, then plans, implements, verifies, and repeats fixes until it passes or blocks. |
