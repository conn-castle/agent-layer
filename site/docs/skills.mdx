...
- **A workflow-bundle library of 25 skills** covering planning, implementation, review, debugging, PR lifecycle, auditing, and codebase improvement — available when the workflow bundle is installed through the wizard and refreshed through upgrade flows for repos that already have it.
...
The Agent Layer workflow bundle includes 25 skills. They are organized here by how you typically use them. Agent Layer also ships optional CLI catalog skills for installed tools; those are listed separately below.
...
| `clean-and-fix-code` | Working-tree cleanup and fix orchestrator. Runs diff-scoped test pruning and new-code simplification as internal assets, then iterates review/fix rounds until convergence. Used as a pre-commit quality gate. |
