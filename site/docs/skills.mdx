...
- **A workflow-bundle library of 10 focused skills** covering implementation, debugging, PR lifecycle, auditing, autonomous delivery, and codebase improvement — available when the workflow bundle is installed through the wizard and refreshed through upgrade flows for repos that already have it.
...
The Agent Layer workflow bundle includes 10 skills. They are organized here by how you typically use them. Agent Layer also ships optional CLI catalog skills for installed tools; those are listed separately below.
...
| Skill | What it does |
| --- | --- |
| `debug-and-fix-issue` | Reproduces an unexplained bug, proves its root cause, captures a failing test or diagnostic blocker, then makes and verifies a proportional repair. |
...
| Skill | What it does |
| --- | --- |
| `improve-codebase` | Runs one broad evidence-led quality sweep, repairs material local and cross-cutting findings, reviews the concrete work, and verifies the result. |
| `simplify-codebase` | Removes material internal complexity such as dead code, obsolete options, duplicate workflows, and misplaced boundaries while preserving external behavior. |
