...
- **Client projection** — User-managed `.agent-layer/skills/` and Git-managed `.agent-layer/imported-skills/` are canonical. `al sync` merges them into one immutable source snapshot, then projects that snapshot to `.agents/skills/` for shared-skill clients and `.claude/skills/` for Claude Code.
...
repository transports. If a destination skill contains an ignored artifact,
