...
above means each named plan reviewer is dispatched once with a distinct review
responsibility through the `agent-review-prompt` asset. Their findings are
synthesized within the same review stage; agreement among reviewers is not an
independent form of evidence. If the asset receives no agent-role inputs, leave
the parentheses off.
...
- `complete-current-phase` orchestrates roadmap work through planning,
  implementation, concrete-work review, verification, and cleanup, with one
  purposeful pass per stage unless concrete evidence requires repair.
- `ship-pr` owns one pull request shipping stage, including audit, commit, push,
  ...
  unrelated stages.
- `full-workflow` owns specification alignment, planning, implementation,
  concrete-work review, verification, and pull request shipping. Each stage
  yields its result before the workflow advances.
- `improve-codebase` decomposes broad quality work into reviewable chunks,
  gives each chunk complementary review, implementation, verification, and
  cleanup stages, and repeats work only after a concrete failure.
- `fix-issues` works through `ISSUES.md` by batching, planning, implementing,
  verifying, and keeping the ledger current without adding review layers merely
  for confidence.
