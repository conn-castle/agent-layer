...
| `review-code` | Review explicit files, diffs, directories, uncommitted changes, or intentionally selected hotspots and produce prioritized findings. |
...
and portable. A workflow can call `review-code` several times with different
scopes; `review-code` should not decide the whole project lifecycle or call
other skills.
