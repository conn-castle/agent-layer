...
**Root skills own one step.** Planning, review, implementation, and verification
are different jobs. Keeping them separate improves activation, reviewability,
and reuse.
...
| `implement-plan` | Execute an approved plan against the codebase, run task-local checks, and record deviations or follow-up. | `implement-plan` |
| `review-code` | Review explicit files, diffs, directories, or uncommitted changes and produce prioritized findings. | `review-scope` |
| `verify-work` | Compare completed work against the plan or request and report completeness, regressions, missing checks, working-code evidence, and scope drift. | `verify-work` |

The current names are the truth for invocation today. Most target names still
describe the desired stable module boundaries as the skill library evolves;
`verify-work` has already moved to its target name.
