...
Agent Layer gives every agent the same instructions, approvals, and MCP servers by generating each client configuration from `.agent-layer/`. Install `al`, initialize the repo, then run `al <client>` to sync and launch.
