...
You can delete generated outputs at any time. Running `al sync` or `al <client>` will recreate them from `.agent-layer/`. The exception is `.agy/antigravity-cli/settings.json`: it is shared state that can contain native workspace approval or trust values, so keep it gitignored but do not delete it as part of cleanup. Agent Layer patches only its managed model, `permissions.allow`, and `agent_specific` paths; its Antigravity MCP output remains disposable.
