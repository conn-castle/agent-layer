...
| `al upgrade plan` | Preview categorized upgrade changes and line-level diffs without writing files (`--json` is optional diagnostic output and not a stable schema contract). | [Upgrade plan](./reference#upgrade-plan) |
