...
Claude Code stores authentication in the OS credential store (macOS Keychain service `"Claude Code-credentials"`; Linux libsecret/gnome-keyring) regardless of `CLAUDE_CONFIG_DIR`. This upstream limitation means `.claude-config/` does not contain auth tokens.
