...
- `.mcp.json` contains resolved Muse MCP values when Muse is enabled and is written with owner-only permissions (`0600`). Sync refuses to write it when tracked or unignored in a Git worktree. Muse authentication and sessions use native user storage; Agent Layer does not isolate them. Historical `.muse-config/` and `.muse-data/` may still contain sensitive state and should be preserved.
...
Treat `.codex/`, `.claude-config/`, `.muse-config/`, and `.muse-data/` as sensitive local state and keep them out of Git. The default template ignores these directories; preserve those rules if you customize the managed gitignore block.
