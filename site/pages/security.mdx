...
- `.muse-config/muse/settings.json` may contain resolved MCP URLs, headers, and environment values; the surrounding `.muse-config/` root may also contain Muse authentication state. `.muse-data/` contains repo-local Muse sessions and runtime data. Both roots are gitignored and owner-only by default.
