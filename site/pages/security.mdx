...
Treat generated client outputs as build artifacts and keep them gitignored. In particular, `.codex/config.toml` may contain resolved values for MCP server URLs, commands, or environment variables. When `local_config_dir = true` is enabled under `[agents.claude]`, the `.claude-config/` directory (set via `CLAUDE_CONFIG_DIR`) may contain authentication state and user-level settings.
