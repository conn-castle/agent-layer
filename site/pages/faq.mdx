...
Yes. Set `.agent-layer/al.version` to `X.Y.Z` or `vX.Y.Z`, or set an initial pin during `al init` with `--version`. You can temporarily override the pin with `AL_VERSION`.
...
Only if you explicitly run `al upgrade --force`. That mode overwrites managed files and deletes unknown files under `.agent-layer` without prompts. Normal `al upgrade` will prompt before changes.
