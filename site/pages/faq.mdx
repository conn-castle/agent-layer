...
## Does Agent Layer install Claude, Codex, Antigravity, Copilot CLI, Grok, Muse, or VS Code?
No. Agent Layer does not install clients. Install Claude, Codex, Antigravity (`agy`), Copilot CLI, Grok, and Muse yourself and put those CLIs on your `PATH`. Install VS Code separately as an application; `al vscode` requires the `code` command on `PATH`.
...
- Agent config: `.mcp.json`, `.agy/antigravity-cli/settings.json`, `.agy/antigravity-cli/mcp_config.json`, `.claude/settings.json`, `.copilot/mcp-config.json`, `.grok/config.toml`, `.muse-config/muse/settings.json`
