#!/usr/bin/env bash
# Scenario 113: Antigravity probe — verifies al probe antigravity runs agy,
...
run_scenario_antigravity_probe() {
  ...
  local output rc=0
  output="$(cd "$repo_dir" && al probe antigravity 2>&1)" || rc=$?
  if [[ $rc -eq 0 ]]; then
    pass "al probe antigravity succeeds with mock agy"
  else
    fail "al probe antigravity should succeed with mock agy (exit code: $rc)"
  ...
  output="$(cd "$repo_dir" && al probe antigravity 2>&1)" || rc=$?
  if [[ $rc -ne 0 ]]; then
    pass "al probe antigravity exits non-zero when agy fails"
  else
    fail "al probe antigravity should exit non-zero when agy fails"
  fi
