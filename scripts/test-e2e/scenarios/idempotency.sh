...
run_scenario_idempotency() {
  ...
  # Bare init has no instruction sources, so .claude/rules/agent-layer.md is expected to be empty.
  assert_file_empty "$repo_dir/.claude/rules/agent-layer.md" \
    ".claude/rules/agent-layer.md is empty before idempotency snapshot without instruction sources"
