...
ENVEOF
  ...
  assert_file_not_exists "$repo_dir/.muse-config/muse/settings.json" "Wizard does not generate redirected Muse settings"
  assert_file_contains "$repo_dir/.mcp.json" '"streamable-http"' \
    "Muse receives HTTP server in all-enabled configuration"
